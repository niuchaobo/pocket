...
class ODHBack {
    ...
    onInstalled(details) {
        if (details.reason === 'install') {
            //安装之后获取二维码
            /*let qr_ticket = getTicketFromBackend().then(value => alert(value["ticket"]));
            let options = optionsLoad();
            options.qr_ticket = qr_ticket;
    ...
    async api_addFavourite(params){
        let {callback} = params;
        try {
            let result = await addfavourite_frontend();
            callback(result);
        } catch (err) {
            callback(null);
        }
    ...
    async findTerm(expression) {
        return new Promise((resolve, reject) => {
            this.agent.postMessage('findTerm', { expression }, result => {
                chrome.storage.local.set({expression:expression}, resolve(result));
            });
        });
    }
