...
class encn_Collins {
    ...
    async findCollins(word) {
        ...
        try {
            ...
            return [].concat(youdao,collins, ydtrans);
        } catch (err) {
            return [];
        }
        ...
        function getCollins(doc) {
            let notes = [];
            //get head overview
            let headoverview = doc.querySelectorAll('.baav .phonetic');

            //get overview
            let overview = html(doc.querySelector('#phrsListTab .trans-container'));
            ...
            let css = `
                <style>
                    ...
                    span.pos  {text-transform:lowercase; font-size:0.9em; margin-right:5px; padding:2px 4px; color:white; background-color:#ffbb00; border-radius:3px;}
                    ...
                    ul {padding-left: 5px; margin-top: 5px;}
                </style>`;
        ...
        function getYoudao(doc) {
            ...
            if (readings) {
                ...
                reads[0] = reading_uk ? `${reading_uk}` :'';
                reads[1] = reading_us ? `${reading_us}` :'';
